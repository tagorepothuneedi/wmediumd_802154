...
//{
//	wqueue_init(&station->queues[IEEE802154_AC_BK], 15, 1023);
//	wqueue_init(&station->queues[IEEE802154_AC_BE], 15, 1023);
//	wqueue_init(&station->queues[IEEE802154_AC_VI], 7, 15);
//	wqueue_init(&station->queues[IEEE802154_AC_VO], 3, 7);
//}
...
static enum ieee802154_ac_number frame_select_queue_802154(struct frame *frame)
{
	u8 *p;
	int priority;

	if (!frame_is_data(frame))
		return IEEE802154_AC_VO;

	if (!frame_is_data_qos(frame))
		return IEEE802154_AC_BE;
...
void deliver_expired_frames(struct wmediumd *ctx)
{
	...
	list_for_each_entry(station, &ctx->stations, list) {
		...
		//	   q_ct[ieee802154_AC_BK], q_ct[ieee802154_AC_BE],
		//	   q_ct[ieee802154_AC_VI], q_ct[ieee802154_AC_VO]);
